  ...
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-button class="filter-item"
                 ...
                 type="primary"
                 icon="el-icon-edit">
        <router-link :to="'/tongzhi/notic/add'">
          添加
        </router-link>
    ...
    <el-table :key="tableKey"
      ...
      <el-table-column label="是否需要确认收到"
                       min-width="130px">
        <template slot-scope="{row}">
          {{row.isQueren == 1?"是":"否"}}
        </template>
      </el-table-column>
      <el-table-column label="是否全部收到"
                       min-width="150px">
        <template slot-scope="{row}">
          <p>{{row.isQueren==1 && row.pan==true?"否":"是"}}</p>
        </template>
      </el-table-column>
      <el-table-column label="Actions"
        ...
        <template slot-scope="{row}">
          <router-link :to="'/tongzhi/notic/edit?id='+row.id">
            <el-button type="text">&emsp;编辑</el-button>
          </router-link>
          |
          <router-link :to="'/tongzhi/notic/xiangxi?id='+row.id">
            <el-button type="text">&emsp;查看详细</el-button>
          </router-link>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    async getList () {
      ...
      var jie = await NoticSv.getNotics(this.listQuery);
