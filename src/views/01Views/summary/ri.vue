<template>
  <div class="app-container">
      ...
      <el-table-column label="Actions"
        ...
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.status!='deleted'"
                     size="mini"
                     type="danger">
            <router-link :to="'/jihua/summary/shenri?id='+row.id">
              查看
            </router-link>
          </el-button>
        </template>
    ...
    <el-dialog :title="textMap[dialogStatus]"
               ...
               width="75%">
      <div style="padding-left: 80px;line-height:35px;font-weight:700">
        <span><span style="color:#606266;">发起人</span> : {{dlr.name}}</span> | <span> <span>日期</span> : {{riqi}}</span>
      </div>
...
export default {
  ...
  methods: {
    async getList () {
      ...
      var jie = await SummarySv.getSummarys(this.listQuery);
      console.log(jie);
      this.list = jie.data;
