<template>
  <div class="app-container">
        ...
        <template slot-scope="{row}">
          <el-tag>
            <span class="link-type"
                  @click="handleUpdate(row,1)">{{row.username}}
            </span>
          </el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]"
      ...
      <div>
        <StaffList :hidden='!setStaff'
                   :isShow="true"
                   :panBu='temp.id'
                   @selectStaff='getSelectStaff' />
      </div>
      <div slot="footer"
           class="dialog-footer">
...
export default {
  ...
  methods: {
    async getList () {
      ...
      var jie = await DepartmentSv.getDepartments(this.listQuery);
      this.list = jie.data;
